    ...
    var body: some View {
        ...
        Pager(page: self.page, data: Array(0..<service.results.count),id: \.self) { index in
            if service.results.count > 0 {
                MovieCardView(imageURL: Constants.baseImageURL + service.results[index].poster_path, backgroundImageURL: Constants.baseLowResImageURL + service.results[index].poster_path, name: service.results[index].original_title, id: service.results[index].id)
            }
        }
